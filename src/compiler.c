...
static void handleLongArgument(OpCode instruction, size_t arg) {
  emitByte(instruction == OP_GET_GLOBAL ? OP_GET_GLOBAL_LONG
                                        : OP_SET_GLOBAL_LONG);
...
static void namedVariable(Token name, bool canAssign) {
  ...
  OpCode instruction = getInstructionForNamedVariable(name, canAssign);

  if (arg >= 256u) {
    handleLongArgument(instruction, arg);
    return;
  }
  emitBytes(instruction, arg);
}
